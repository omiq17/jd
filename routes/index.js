...
var url = 'mongodb://omiq:1700@ds057386.mlab.com:57386/jd';
...
router.get('/programs', function(req, res, next) {
  ...
  mongo.connect(url, function(err, db) {
    ...
    cursor.forEach(function(doc, err) {
      ...
      db.close();
      res.render('programs', {page: '/programs', items: result });
    });
  });
});
...
// for Archive
router.post('/insertArchive', function(req, res, next){
  var item = {
    name : req.body.arcName,
    type : req.body.arcType,
    desc: req.body.arcDes,
    link: req.body.arcLink,
    prog_id: req.body.progId,
    video: req.body.arcVid
  }; 

  mongo.connect(url, function(err, db){
    assert.equal(null, err);
    db.collection('archives').insertOne(item, function(err, result){
    assert.equal(err, null);
    db.close();
    });
  });
